...
local function InsertWarning(identifier, name, reason)
    local insertData = MySQL.Sync.fetchAll('INSERT INTO `esx_manager_warns` (`warned`, `name`, `reason`, `date`) VALUES(@w, @n, @r, @d)',{
        ['@w'] = identifier,
        ['@n'] = name,
        ['@r'] = reason,
...
function UpdateWarning(warningId, reason)
    if warnList[tostring(warningId)] then
        MySQL.Sync.execute('UPDATE `esx_manager_warns` SET `reason` = @r WHERE `id` = @i', {
            ['@r'] = reason,
            ['@i'] = warningId
        })
        warnList[tostring(warningId)].reason = reason
        return true
    end
    return false
end
...
function GetWarnList(source)
    if not ready then ReloadWarnings() end
    if not source then
        return warnList
    else
        local warns = {}
