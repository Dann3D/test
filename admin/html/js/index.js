...
const main = new Vue({
    ...
    methods: {
        ...
        give(type) {
            if (type === "item") {
                    ...
                    if (data.executed) {
                        ...
                        for (const index in main.selected.inventory) {
                            ...
                            if (item.name === data.name) {
                                main.selected.inventory[index].count = data.count;
                            }
                        }
                        main.$forceUpdate()
            ...
            else if (type === "money" || type === "bank" || type === "black_money") {
                ...
                post('/admin/giveWeapon', {
                    source: this.selected.source,
                    name: this.$refs.weapon_name.value.toUpperCase(),
                    ammo: this.$refs.weapon_ammo.value
                }, (data) => {
                    if (data.executed){
        ...
        remove(type) {
            if (type === "item") {
                ...
                post('/admin/removeWeapon', {
                    source: this.selected.source,
                    name: this.$refs.weapon_name.value.toUpperCase(),
                    ammo: this.$refs.weapon_ammo.value || 0
                }, (data) => {
                    if (data.executed){
        ...
        set(type) {
            if (type === "money" || type === "bank" || type == "black_money") {
        ...
        runglobal(type) {
            if (type === "heal") {
                post('/admin/globalHeal', {},
                (data) => {
                    if (!data.executed) main.showErrorMessage(data.message || "Error");
                })
            }
            else if (type === "revive") {
                post('/admin/globalRevive', {},
                (data) => {
                    if (!data.executed) main.showErrorMessage(data.message || "Error");
                })
            }
        ...
        kick(index, message) {
            post('/admin/kick', {
                source: index,
                message: message || "Kicked from server."
            }, (data) => {
                if (data.executed === true) {
                    main.$refs.message.value = '';
                    main.removePlayerFormList(data.source);
                } else main.showErrorMessage(data.message || "Error");
            });
        },
        ban(message) {
